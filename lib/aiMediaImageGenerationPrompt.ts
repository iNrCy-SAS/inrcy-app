      ...
      ? [
          ...
          inspirationReferences.length
            ? `INSPIRATION UNIQUEMENT : ${inspirationReferences.length} média${
              ...
              } librement l’ambiance, la palette ou la composition. Ne pas imposer leur identité, leur visage, leur silhouette, leur produit ou leur décor exact.`
            : "",
        ]
          .filter(Boolean)
          .join("\n")
  ...
  if (args.deferVisibleElementsToComposer) {
    const localTextMode =
      textMode === "exact" && exactVisibleText
        ? `MODE TEXTE EXACT : iNrCy composera localement et exclusivement « ${exactVisibleText} », sans reformulation, traduction, correction, ajout ni omission ; aucun autre titre, slogan, sous-titre, enseigne, CTA ou texte ne sera ajouté.`
        : textMode === "ai" && aiVisibleHeadline
        ? `MODE TEXTE IA CONTEXTUALISÉ : iNrCy composera localement et exclusivement l’accroche validée « ${aiVisibleHeadline} », dérivée des faits et du sujet, différente de l’historique récent. Clichés interdits : ${AI_MEDIA_GENERIC_HEADLINE_BLACKLIST}.`
...
export function buildAiMediaImageGenerationPrompt(
  ...
  const compiledPrompt = [
    ...
    request.inputMode !== "essential"
      ...
      : `MODE STUDIO GUIDÉ IMAGE : typologie ${request.typology}, style ${request.visualStyle}, rendu ${request.imageStyle}, cadrage ${request.shotType}, présence ${request.peopleMode}, créativité ${request.creativity}. Le brief reste prioritaire et l’ADN sert uniquement de contexte.`,
