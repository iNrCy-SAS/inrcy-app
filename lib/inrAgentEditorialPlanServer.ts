...
export async function notifyReadyInrAgentEditorialBatch(args: {
  ...
  const result = await insertNotificationOnce({
    ...
    title: "Vos publications iNrAgent sont prêtes",
    body: `iNrAgent a préparé ${count} publication${count > 1 ? "s" : ""} pour les ${horizonDays} prochains jours. Contrôlez-les puis validez-les avant leur diffusion.`,
