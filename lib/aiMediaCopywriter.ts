...
import {
  ...
  extractAiMediaRequestedDialogue,
  isQualityAiMediaDialogueLine,
  selectAiMediaDialogueLine,
  validateAiMediaRequestedDialogue,
  ...
  schema: {
    ...
    properties: {
      headline: {
        type: "string",
        minLength: 3,
        maxLength: AI_MEDIA_VISIBLE_TITLE_MAX_CHARACTERS,
      },
        ...
        items: {
          ...
          properties: {
            eyebrow: {
              type: "string",
              minLength: 1,
              maxLength: AI_MEDIA_VISIBLE_EYEBROW_MAX_CHARACTERS,
            },
            title: {
              type: "string",
              minLength: 2,
            ...
            body: {
              type: "string",
              minLength: 0,
              maxLength: AI_MEDIA_VISIBLE_BODY_MAX_CHARACTERS,
            },
            spokenLine: {
              type: "string",
              minLength: 2,
              maxLength: AI_MEDIA_SPOKEN_LINE_MAX_CHARACTERS,
            },
            spokenReply: {
              type: "string",
              minLength: 2,
...
function isGenericMediaCopy(value: unknown) {
  ...
  return GENERIC_MEDIA_COPY_PATTERNS.some((pattern) =>
...
function recoverCopywriterFailure(args: {
  ...
  if (
    ...
    !hasQualityGeneratedDialogueDeck({
      generated: plan,
    ...
    const recovered = recoverLocalDialogue(plan, args.language, args.idea);
    if (!recovered) throw new Error("ai_media_character_dialogue_unavailable");
    plan = recovered;
  }
  if (args.textMode !== "ai") {
    return removeAiMediaFallbackCopy(plan, {
...
function recoverLocalDialogue(
  plan: AiMediaCreativePlan,
  language: string,
  idea: string,
): AiMediaCreativePlan | null {
  const facts = [idea, plan.headline, ...plan.scenes.flatMap((scene) => [
    scene.spokenLine, scene.body, scene.title,
  ])].flatMap((value) => [value, ...(value.match(/[^.!?]+[.!?]+/gu) || [])]);
  const used = new Set<string>();
  const scenes: AiMediaCreativePlan["scenes"] = [];
  for (const [sceneIndex, scene] of plan.scenes.entries()) {
    const spokenLine = [scene.spokenLine, scene.body, scene.title, ...facts].find(
      (candidate) =>
        isQualityAiMediaDialogueLine(candidate, language, used) &&
        headlineRespectsIdea(candidate, idea, language === "fr"),
    );
    if (!spokenLine) return null;
    used.add(aiMediaDialogueSignature(spokenLine));
    ...
    used.add(aiMediaDialogueSignature(spokenReply));
    scenes.push({ ...scene, spokenLine, spokenReply });
  }
  return { ...plan, scenes };
}
...
function compactCompleteBody(
  value: unknown,
  maximum = AI_MEDIA_VISIBLE_BODY_MAX_CHARACTERS
) {
  return acceptCompleteAiMediaVisibleCopy(value, maximum);
}
...
function headlineRespectsIdea(
  ...
  const ideaLooksLikeInstruction =
    /^(?:je\s+(?:veux|souhaite|voudrais)\b|(?:cr[eé]er|faire|mettre|montrer|pr[eé]senter|illustrer|raconter|expliquer|valoriser|animer|filmer)\b)/i.test(
      String(idea || "").trim()
    );
...
function applyLocalizedCopy(
  ...
  const scenes = plan.scenes.map((scene, index) => {
    ...
    const title =
      titleSignature && !usedTitles.has(titleSignature) ? generatedTitle : "";
  ...
  const subline = compactCompleteBody(scenes[0]?.body);
  const visibleCopy = [
    headline,
    subline,
    cta,
  ...
  if (
    ...
    !preservesAiMediaProtectedTerms(visibleCopy, protectedTerms)
  ) {
    return null;
  }
...
export async function writeAiMediaHeadline(args: {
  ...
  const exactDialogue = characterDialogueRequested
    ? extractAiMediaRequestedDialogue([args.request.idea, args.request.aiInstruction].filter(Boolean).join("\n"))
    : [];
  if (exactDialogue.length) {
    validateAiMediaRequestedDialogue(exactDialogue, args.profile.preferences.language);
    if (exactDialogue.length > args.plan.scenes.length) throw new Error("ai_media_exact_dialogue_unfit");
    args = { ...args, plan: { ...args.plan, scenes: args.plan.scenes.map((scene, index) => ({
      ...scene, spokenLine: exactDialogue[index] || "", spokenReply: "",
    })) } };
    // Un dialogue fourni mot pour mot n'a pas besoin d'une rédaction IA.
    // Le plan visuel et toutes ses intentions restent strictement inchangés.
    if (textMode === "none") return removeAiMediaFallbackCopy(args.plan, { keepSpokenCopy: true });
  }
  ...
  }).filter((term) => !exactDialogue.some((line) =>
    normalizeAiMediaCopy(line).replace(/[.!?]+$/u, "") === normalizeAiMediaCopy(term).replace(/[.!?]+$/u, ""),
  ));

  try {
    const qualityAttempts = characterDialogueRequested
      ? 3
      : textMode === "ai"
      ? 2
      : 1;
    ...
    for (
      ...
      qualityAttempt < qualityAttempts;
      qualityAttempt += 1
    ) {
      const generated = await aiGenerateJSON<GeneratedMediaCopy>({
        ...
        system: [
          ...
          `spokenReply est une réponse courte et complète de 5 à ${AI_MEDIA_SPOKEN_LINE_MAX_WORDS} mots et ${AI_MEDIA_SPOKEN_LINE_MAX_CHARACTERS} caractères maximum qui poursuit naturellement spokenLine pour une éventuelle seconde personne.`,
          ...
          "Si un brouillon rejeté est fourni, réécris entièrement ses phrases : ne le corrige pas mot par mot et n'en conserve aucune succession de mots.",
          ...
          protectedTerms.length
            ? `TERMES LITTÉRAUX OBLIGATOIRES : ${protectedTerms.join(
                " ; "
              )}. Chaque expression est atomique : recopie-la exactement, sans la traduire, la raccourcir, la séparer ni en supprimer le dernier mot. Réécris les mots autour si nécessaire.`
        ...
        input: JSON.stringify({
          ...
          brouillon_rejete_a_ne_pas_reprendre: rejectedCopy || null,
        }),
        responseSchema: MEDIA_COPY_SCHEMA,
        // Les modèles GPT-5.x incluent les jetons de raisonnement dans cette
        // enveloppe. Le JSON complet d'une vidéo à trois scènes a donc besoin
        // d'une marge supérieure au deck image mono-scène.
        maxOutputTokens: args.request.kind === "video" ? 1_536 : 1_024,
        temperature: args.request.creativity === "bold" ? 0.88 : 0.65,
        retries: 0,
        // Chaque tentative reste bornée. Le contrôle local peut demander une
        // nouvelle rédaction dans le même clic, avant tout appel au fournisseur
        // média, sans jamais recycler un brouillon incomplet ou une liste de mots.
        timeoutMs: 30_000,
        deadlineAt: Date.now() + 31_000,
      });
      const validated = validateGeneratedMediaCopy({
        generated: exactDialogue.length && Array.isArray(generated.scenes)
          ? { ...generated, scenes: generated.scenes.map((scene, index) => exactDialogue[index]
            ? { ...scene, spokenLine: exactDialogue[index] }
            : scene) }
          : generated,
        request: args.request,
        plan: args.plan,
      ...
      rejectedCopy = summarizeRejectedCopy(generated);
    }
    ...
    if (
      ...
      ["ai_media_visible_copy_unavailable", "ai_media_character_dialogue_unavailable"].includes(error.message)
    ) {
      throw error;
    }
    return recoverCopywriterFailure({
...
function hasQualityGeneratedDialogueDeck(args: {
  generated: GeneratedMediaCopy;
  language: string;
  sceneCount: number;
  idea: string;
}) {
  const scenes = Array.isArray(args.generated.scenes)
    ? args.generated.scenes
    : [];
  if (scenes.length !== args.sceneCount) return false;

  const used = new Set<string>();
  return scenes.every((scene) => {
    if (!isQualityAiMediaDialogueLine(scene?.spokenLine, args.language, used)) {
      return false;
    }
    if (!headlineRespectsIdea(String(scene?.spokenLine || ""), args.idea, args.language === "fr")) {
      return false;
    }
    used.add(aiMediaDialogueSignature(scene?.spokenLine));
    if (!isQualityAiMediaDialogueLine(scene?.spokenReply, args.language, used)) {
      return false;
    }
    used.add(aiMediaDialogueSignature(scene?.spokenReply));
    return true;
  });
}

function summarizeRejectedCopy(generated: GeneratedMediaCopy) {
...
function validateGeneratedMediaCopy(args: {
  generated: GeneratedMediaCopy;
  request: AiMediaGenerationRequest;
  plan: AiMediaCreativePlan;
  language: NormalizedAiGenerationProfile["preferences"]["language"];
