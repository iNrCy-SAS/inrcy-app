...
where routine_schema = 'public'
  and routine_name = 'inrcy_set_instagram_publication_preferences';
...
from (
  values (
    to_regprocedure(
      'public.inrcy_set_instagram_publication_preferences(uuid,jsonb)'
    )
  )
...
select
  coalesce(
    ...
    false
  ) as authenticated_select_is_account_scoped,
  coalesce(
    bool_or(
      cmd in ('INSERT', 'ALL')
      and ('authenticated' = any(roles) or 'public' = any(roles))
    ...
    false
  ) as authenticated_insert_is_account_scoped,
  coalesce(
    bool_or(
      cmd in ('UPDATE', 'ALL')
      and ('authenticated' = any(roles) or 'public' = any(roles))
    ...
    false
  ) as authenticated_update_is_account_scoped
from pg_policies
where schemaname = 'public'
  and tablename = 'pro_tools_configs';
