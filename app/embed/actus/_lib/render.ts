...
function getImageOverlay(article: Record<string, unknown>, index: number): ImageOverlay | undefined {
  const metadata = parseJsonRecord(article.media_metadata);
  const overlays = metadata.imageOverlays;
  if (!Array.isArray(overlays)) return undefined;
  const entry = readRecord(overlays[index]);
  return normalizeImageOverlay(entry.overlay || entry);
}
...
function renderMediaBlock(article: Record<string, unknown>, idPrefix: string) {
  ...
  if (images.length === 1) {
    ...
    return `
      <div class="mediaCol" data-natural-media-frame>
        ${wrapImageLink(image, getImageOverlay(article, 0))}
  ...
  const slides = images.map((img, imageIndex) => `
    <div class="mediaSlide" data-media-slide>
      ${wrapImageLink(`<img class="media" src="${safeAttr(stableImageSrc(img))}" data-original-src="${safeAttr(img)}" alt="" loading="eager" fetchpriority="high" decoding="async" referrerpolicy="no-referrer" />`, getImageOverlay(article, imageIndex))}
    </div>
  `).join("");
