...
function getImageInteractionOverlay(
  article: Record<string, unknown>,
  index: number,
): ImageOverlay | undefined {
  const metadata = parseJsonRecord(article.media_metadata);
  const interactions = metadata.imageInteractions || metadata.image_interactions;
  let studioInteractions: ImageInteractions | undefined;
  if (Array.isArray(interactions)) {
    const entry = readRecord(interactions[index]);
    const normalized = normalizeImageInteractions(entry.interactions || entry);
    if (normalized) studioInteractions = normalized;
  }

  // Compatibilité avec les publications antérieures au contrat
  // `imageInteractions`, où le lien vivait encore dans l'overlay de rendu.
  const overlays = metadata.imageOverlays || metadata.image_overlays;
  const overlayEntry = Array.isArray(overlays)
    ? readRecord(overlays[index])
    : {};
  return mergeImageInteractionOverlays(
    studioInteractions,
    overlayEntry.overlay || overlayEntry,
  );
}

function wrapImageLinks(content: string, overlay: ImageOverlay | undefined) {
  const links = getImageInteractionLinkHotspots(overlay);
  if (!links.length) return content;

  const preciseLinks = links
    .map((link, index) => {
      if (!link.geometry) return "";
      const { x, y, width, height } = link.geometry;
      return `<a class="mediaLinkHotspot" data-media-link-hotspot="${index}" href="${safeAttr(link.url)}" target="_blank" rel="noopener noreferrer" aria-label="${safeAttr(link.label)}" title="${safeAttr(link.label)}" style="left:${x}%;top:${y}%;width:${width}%;height:${height}%"></a>`;
    })
    .filter(Boolean)
    .join("");

  // Les anciennes métadonnées ne connaissaient qu'un lien global. On conserve
  // ce comportement en arrière-plan ; les zones précises restent prioritaires.
  const legacyLink = links.find((link) => !link.geometry);
  const legacyAnchor = legacyLink
    ? `<a class="mediaLinkLegacy" data-media-link-legacy href="${safeAttr(legacyLink.url)}" target="_blank" rel="noopener noreferrer" aria-label="${safeAttr(legacyLink.label)}" title="${safeAttr(legacyLink.label)}"></a>`
    : "";

  return `<div class="mediaInteractive">${content}${legacyAnchor}${preciseLinks}</div>`;
}
...
function renderMediaBlock(article: Record<string, unknown>, idPrefix: string) {
  ...
  if (images.length === 1) {
    const image = `<img class="media" src="${safeAttr(stableImageSrc(images[0]))}" data-original-src="${safeAttr(images[0])}" alt="" loading="eager" fetchpriority="high" decoding="async" referrerpolicy="no-referrer" />`;
    return `
      <div class="mediaCol" data-natural-media-frame>
        ${wrapImageLinks(image, getImageInteractionOverlay(article, 0))}
  ...
  const slides = images.map((img, imageIndex) => `
    <div class="mediaSlide" data-media-slide>
      ${wrapImageLinks(`<img class="media" src="${safeAttr(stableImageSrc(img))}" data-original-src="${safeAttr(img)}" alt="" loading="eager" fetchpriority="high" decoding="async" referrerpolicy="no-referrer" />`, getImageInteractionOverlay(article, imageIndex))}
    </div>
  `).join("");
